...
def get_parser():
    ...
    parser.add_argument('-temp', help='temporal folder', default='/no_backup/rg/dsantesmasses/temp/')
    ...
    database.add_argument('-db', '--database',
                        ...
                        default='/users/rg/didac/NCBI/Taxonomy/tax.db')
...
def get_taxid_from_names(arg, temp_dir, unmask, is_list=False, quiet=False):
    ...
    for k,v in out_hash.items():
        taxids_from_input_dict.setdefault(v,[]).append(k)
    for x,v in taxids_from_input_dict.items():
        if len(v)>1:
...
def get_name_and_annotation(arg, temp_dir):
    ...
    taxid_attrs = {}
    for name,taxid in out_hash.items():
...
#    taxid_list = list(set(taxid_list))
    ...
    if args.taxid_annotation or args.name_annotation:
        for taxid, attrs_dict in taxid_attrs.items():
            try:
    ...
    if not args.no_name:
        if args.name:
            for taxid,names_list in taxids_from_input_dict.items():
    ...
    if args.fix_internal and internal_nodes_in_input:
        for node in internal_nodes_in_input:
...
Returns a list where each item has (taxid, parent_taxid, rank, name)"""
    n = 999
    results = []
    for i in xrange(0, len(taxid_list), n):
